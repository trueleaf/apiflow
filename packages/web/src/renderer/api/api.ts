...
axiosInstance.interceptors.response.use(
  async (res: AxiosResponse) => {
    ...
    if (contentType.includes('application/json')) { //常规格式数据
      ...
      switch (code) {
        ...
        case 1020: //分享链接不存在
          ElMessage({
            message: i18next.t('分享链接不存在'),
            grouping: true,
            type: 'warning',
          });
          return Promise.reject(new Error(i18next.t('分享链接不存在')));
        case 1021: //分享链接已过期
          ElMessage({
            message: i18next.t('分享链接已过期'),
            grouping: true,
            type: 'warning',
          });
          return Promise.reject(new Error(i18next.t('分享链接已过期')));
        case 1022: //分享链接无需密码
          ElMessage({
            message: i18next.t('分享链接无需密码'),
            grouping: true,
            type: 'warning',
          });
          return Promise.reject(new Error(i18next.t('分享链接无需密码')));
        case 1023: //密码错误
          ElMessage({
            message: i18next.t('密码错误'),
            grouping: true,
            type: 'warning',
          });
          return Promise.reject(new Error(i18next.t('密码错误')));
        case 4101: //登录有错
          router.replace('/login');
          ElMessage.warning(i18next.t('暂无权限'));
          return Promise.reject(new Error(i18next.t('暂无权限')));
        case 4100: //登录过期
          if (!isExpire) {
            isExpire = true;
            ElMessageBox.confirm(i18next.t('登录已过期'), i18next.t('提示'), {
              confirmButtonText: i18next.t('跳转登录'),
          ...
          return Promise.reject(new Error(i18next.t('登录已过期')));
        case 4200: //代理错误
          return Promise.reject(new Error(res.data.msg));
        case 4002: //暂无权限
          ElMessage.warning(i18next.t(res.data.msg || '暂无权限'));
          return Promise.reject(new Error(i18next.t(res.data.msg || '暂无权限')));
        default:
          ElMessageBox.confirm(i18next.t(res.data.msg ? res.data.msg : '操作失败'), i18next.t('提示'), {
            confirmButtonText: i18next.t('确定'),
            showCancelButton: false,
            type: 'warning',
          });
          return Promise.reject(new Error(i18next.t(res.data.msg)));
      }
      return result;
    }
  ...
  (err: AxiosError) => {
    ...
    if (err.code === 'ECONNABORTED' || err.message.includes('timeout')) {
      ElMessage({
        message: i18next.t('接口调用超时，请稍后重试'),
        grouping: true,
        type: 'error',
      })
