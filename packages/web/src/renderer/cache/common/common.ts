...
|--------------------------------------------------------------------------
*/
const MOCK_TEXT_RANDOM_SIZE_HINT_KEY = 'hint/mockTextRandomSizeHint'

// 获取 Mock Text 随机大小提示是否可见
export const getMockTextRandomSizeHintVisible = (): boolean => {
  try {
    const value = localStorage.getItem(MOCK_TEXT_RANDOM_SIZE_HINT_KEY)
    // 未设置时默认显示（返回 true）
    if (value === null) {
      return true
    }
    ...
    console.error('获取 Mock Text 随机大小提示状态失败:', error)
    return true // 出错时默认显示
  }
}

// 设置 Mock Text 随机大小提示是否可见
export const setMockTextRandomSizeHintVisible = (visible: boolean): void => {
  try {
    localStorage.setItem(MOCK_TEXT_RANDOM_SIZE_HINT_KEY, visible ? 'true' : 'false')
  } catch (error) {
