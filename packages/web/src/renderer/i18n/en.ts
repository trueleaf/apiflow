export default {
  ...
  log: {
    ...
    '请输入 DeepSeek API Key': 'Please enter DeepSeek API Key',
    'API 地址': 'API Address',
    '超时时间(ms)': 'Timeout (ms)',
    '请输入超时时间': 'Please enter timeout',
    '保存配置': 'Save Config',
    '文本测试': 'Text Test',
    'JSON测试': 'JSON Test',
    '流式测试': 'Stream Test',
    '点击"文本测试"、"JSON测试"或"流式测试"按钮查看大模型响应': 'Click "Text Test", "JSON Test" or "Stream Test" button to view AI model response',
    '正在请求中...': 'Requesting...',
    '配置保存成功': 'Configuration saved successfully',
    '配置保存失败': 'Configuration save failed',
    '请先配置 API Key 和 API 地址': 'Please configure API Key and API Address first',
    ...
    // Component Library
    '基于 Monaco Editor 的代码编辑器，支持 JavaScript/TypeScript，自动补全、语法高亮、主题切换': 'Code editor based on Monaco Editor, supports JavaScript/TypeScript, auto-completion, syntax highlighting, theme switching',
    'CodeEditor 组件演示': 'CodeEditor Component Demo',
    '基于 Monaco Editor 的代码编辑器组件，支持多种编程语言、代码高亮、自动补全、主题切换等功能': 'Code editor component based on Monaco Editor, supporting multiple programming languages, syntax highlighting, auto-completion, theme switching and more',
    '基础用法': 'Basic Usage',
    '支持 JavaScript 和 TypeScript 语言': 'Supports JavaScript and TypeScript',
    'TypeScript 支持': 'TypeScript Support',
    '切换到 TypeScript 语言模式': 'Switch to TypeScript language mode',
    '只读模式': 'Read-only Mode',
    '禁止编辑，仅用于展示代码': 'Editing disabled, for code display only',
    '带格式化按钮': 'With Format Button',
    '显示格式化按钮，一键格式化代码': 'Show format button for one-click code formatting',
    '固定高度': 'Fixed Height',
    '不使用自动高度，设置固定容器高度': 'No auto height, set fixed container height',
    '带占位符': 'With Placeholder',
    '编辑器为空时显示占位符文本': 'Show placeholder text when editor is empty',
    'Props 属性': 'Props',
    '属性名': 'Property',
    'Events 事件': 'Events',
    '事件名': 'Event',
    ...
    '是否自动高度': 'Whether auto height',
    '最小高度': 'Min height',
    '最大高度': 'Max height',
    '显示格式化按钮': 'Show format button',
    '占位符文本': 'Placeholder text',
    '禁用语法校验': 'Disable validation',
    '编辑器配置': 'Editor config',
    '内容改变时触发': 'Triggered when content changes',
    '编辑器初始化完成': 'Editor initialization completed',
    '基于 Monaco Editor，提供强大的代码编辑能力': 'Based on Monaco Editor, provides powerful code editing capabilities',
    '自动适配亮色/暗色主题': 'Auto adapts to light/dark themes',
    '支持代码高亮、自动补全、语法检查': 'Supports syntax highlighting, auto-completion, syntax checking',
    '支持自动高度或固定高度模式': 'Supports auto height or fixed height mode',
    '支持只读模式': 'Supports read-only mode',
    '内置代码格式化功能': 'Built-in code formatting',
    '支持自定义配置和代码提示': 'Supports custom config and code suggestions',
    '完整的 TypeScript 类型定义': 'Complete TypeScript type definitions',
    '请输入 JavaScript 代码...': 'Please enter JavaScript code...',
}
}
