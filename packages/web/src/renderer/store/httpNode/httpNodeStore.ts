...
export const useHttpNode = defineStore('httpNode', () => {
  ...
  */
  ...
  const patchHttpNodeInfoById = async (nodeId: string, updates: DeepPartial<HttpNode>): Promise<HttpNode | null> => {
    ...
    if (httpNodeInfo.value._id === nodeId) {
      merge(httpNodeInfo.value, updates);
      httpNodeInfo.value.updatedAt = updatedNode.updatedAt;
  ...
  const addQueryParamByNodeId = async (nodeId: string, param: ApidocProperty<'string'>): Promise<HttpNode | null> => {
    ...
    if (httpNodeInfo.value._id === nodeId) {
      httpNodeInfo.value.item.queryParams.push(param);
      httpNodeInfo.value.updatedAt = existingNode.updatedAt;
      changeOriginHttpNodeInfo();
    }
    return existingNode;
  }
  ...
  const updateQueryParamByNodeId = async (nodeId: string, paramId: string, updates: Partial<ApidocProperty<'string'>>): Promise<HttpNode | null> => {
    ...
    if (httpNodeInfo.value._id === nodeId) {
      const localIndex = httpNodeInfo.value.item.queryParams.findIndex(p => p._id === paramId);
      if (localIndex !== -1) {
        assign(httpNodeInfo.value.item.queryParams[localIndex], updates);
      }
      httpNodeInfo.value.updatedAt = existingNode.updatedAt;
      changeOriginHttpNodeInfo();
    }
  ...
  const deleteQueryParamByNodeId = async (nodeId: string, paramId: string): Promise<HttpNode | null> => {
    ...
    if (httpNodeInfo.value._id === nodeId) {
      const localIndex = httpNodeInfo.value.item.queryParams.findIndex(p => p._id === paramId);
      if (localIndex !== -1) {
        httpNodeInfo.value.item.queryParams.splice(localIndex, 1);
      }
      httpNodeInfo.value.updatedAt = existingNode.updatedAt;
      changeOriginHttpNodeInfo();
    }
  ...
  const addPathParamByNodeId = async (nodeId: string, param: ApidocProperty<'string'>): Promise<HttpNode | null> => {
    ...
    if (httpNodeInfo.value._id === nodeId) {
      httpNodeInfo.value.item.paths.push(param);
      httpNodeInfo.value.updatedAt = existingNode.updatedAt;
      changeOriginHttpNodeInfo();
    }
    return existingNode;
  ...
  const updatePathParamByNodeId = async (nodeId: string, paramId: string, updates: Partial<ApidocProperty<'string'>>): Promise<HttpNode | null> => {
    ...
    if (httpNodeInfo.value._id === nodeId) {
      const localIndex = httpNodeInfo.value.item.paths.findIndex(p => p._id === paramId);
      if (localIndex !== -1) {
        assign(httpNodeInfo.value.item.paths[localIndex], updates);
      }
      httpNodeInfo.value.updatedAt = existingNode.updatedAt;
      changeOriginHttpNodeInfo();
    }
  ...
  const deletePathParamByNodeId = async (nodeId: string, paramId: string): Promise<HttpNode | null> => {
    ...
    if (httpNodeInfo.value._id === nodeId) {
      const localIndex = httpNodeInfo.value.item.paths.findIndex(p => p._id === paramId);
      if (localIndex !== -1) {
        httpNodeInfo.value.item.paths.splice(localIndex, 1);
      }
      httpNodeInfo.value.updatedAt = existingNode.updatedAt;
      changeOriginHttpNodeInfo();
    }
  ...
  const addHeaderByNodeId = async (nodeId: string, header: ApidocProperty<'string'>): Promise<HttpNode | null> => {
    ...
    if (httpNodeInfo.value._id === nodeId) {
      httpNodeInfo.value.item.headers.push(header);
      httpNodeInfo.value.updatedAt = existingNode.updatedAt;
      changeOriginHttpNodeInfo();
    }
    return existingNode;
  ...
  const updateHeaderByNodeId = async (nodeId: string, headerId: string, updates: Partial<ApidocProperty<'string'>>): Promise<HttpNode | null> => {
    ...
    if (httpNodeInfo.value._id === nodeId) {
      const localIndex = httpNodeInfo.value.item.headers.findIndex(h => h._id === headerId);
      if (localIndex !== -1) {
        assign(httpNodeInfo.value.item.headers[localIndex], updates);
      }
      httpNodeInfo.value.updatedAt = existingNode.updatedAt;
      changeOriginHttpNodeInfo();
    }
  ...
  const deleteHeaderByNodeId = async (nodeId: string, headerId: string): Promise<HttpNode | null> => {
    ...
    if (httpNodeInfo.value._id === nodeId) {
      const localIndex = httpNodeInfo.value.item.headers.findIndex(h => h._id === headerId);
      if (localIndex !== -1) {
        httpNodeInfo.value.item.headers.splice(localIndex, 1);
      }
      httpNodeInfo.value.updatedAt = existingNode.updatedAt;
      changeOriginHttpNodeInfo();
    }
  ...
  const addFormdataByNodeId = async (nodeId: string, param: ApidocProperty<'string' | 'file'>): Promise<HttpNode | null> => {
    ...
    if (httpNodeInfo.value._id === nodeId) {
      httpNodeInfo.value.item.requestBody.formdata.push(param);
      httpNodeInfo.value.updatedAt = existingNode.updatedAt;
      changeOriginHttpNodeInfo();
    }
    return existingNode;
  ...
  const updateFormdataByNodeId = async (nodeId: string, paramId: string, updates: Partial<ApidocProperty<'string' | 'file'>>): Promise<HttpNode | null> => {
    ...
    if (httpNodeInfo.value._id === nodeId) {
      const localIndex = httpNodeInfo.value.item.requestBody.formdata.findIndex(p => p._id === paramId);
      if (localIndex !== -1) {
        assign(httpNodeInfo.value.item.requestBody.formdata[localIndex], updates);
      }
      httpNodeInfo.value.updatedAt = existingNode.updatedAt;
      changeOriginHttpNodeInfo();
    }
  ...
  const deleteFormdataByNodeId = async (nodeId: string, paramId: string): Promise<HttpNode | null> => {
    ...
    if (httpNodeInfo.value._id === nodeId) {
      const localIndex = httpNodeInfo.value.item.requestBody.formdata.findIndex(p => p._id === paramId);
      if (localIndex !== -1) {
        httpNodeInfo.value.item.requestBody.formdata.splice(localIndex, 1);
      }
      httpNodeInfo.value.updatedAt = existingNode.updatedAt;
      changeOriginHttpNodeInfo();
    }
  ...
  const addUrlencodedByNodeId = async (nodeId: string, param: ApidocProperty<'string'>): Promise<HttpNode | null> => {
    ...
    if (httpNodeInfo.value._id === nodeId) {
      httpNodeInfo.value.item.requestBody.urlencoded.push(param);
      httpNodeInfo.value.updatedAt = existingNode.updatedAt;
      changeOriginHttpNodeInfo();
    }
    return existingNode;
  ...
  const updateUrlencodedByNodeId = async (nodeId: string, paramId: string, updates: Partial<ApidocProperty<'string'>>): Promise<HttpNode | null> => {
    ...
    if (httpNodeInfo.value._id === nodeId) {
      const localIndex = httpNodeInfo.value.item.requestBody.urlencoded.findIndex(p => p._id === paramId);
      if (localIndex !== -1) {
        assign(httpNodeInfo.value.item.requestBody.urlencoded[localIndex], updates);
      }
      httpNodeInfo.value.updatedAt = existingNode.updatedAt;
      changeOriginHttpNodeInfo();
    }
  ...
  const deleteUrlencodedByNodeId = async (nodeId: string, paramId: string): Promise<HttpNode | null> => {
    ...
    if (httpNodeInfo.value._id === nodeId) {
      const localIndex = httpNodeInfo.value.item.requestBody.urlencoded.findIndex(p => p._id === paramId);
      if (localIndex !== -1) {
        httpNodeInfo.value.item.requestBody.urlencoded.splice(localIndex, 1);
      }
      httpNodeInfo.value.updatedAt = existingNode.updatedAt;
      changeOriginHttpNodeInfo();
    }
