...
test.describe('SettingsNavigation', () => {
  test('点击设置按钮创建Tab并跳转设置页面', async ({ topBarPage, contentPage, clearCache }) => {
    await clearCache();
  ...
  test('在项目工作区点击设置能正常跳转', async ({ topBarPage, contentPage, clearCache, createProject }) => {
    await clearCache();
  ...
  test('关闭设置Tab后再次点击设置按钮重新创建', async ({ topBarPage, contentPage, clearCache }) => {
    await clearCache();
    const settingsBtn = topBarPage.locator('[data-testid="header-settings-btn"]');
    // 打开设置
    await settingsBtn.click();
  ...
  test('设置Tab图标显示正确', async ({ topBarPage, contentPage, clearCache, jumpToSettings }) => {
    await clearCache();
