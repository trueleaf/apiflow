...
test.describe('CookieManagement', () => {
  ...
  test('按名称搜索Cookie-精确搜索、模糊搜索、无结果、清空搜索', async ({ contentPage, clearCache, createProject }) => {
    await clearCache();
    await createProject();
    await contentPage.waitForURL(/.*?#?\/workbench/, { timeout: 5000 });
    // 打开Cookie管理页面
  ...
  test('按域名筛选Cookie-特定域名、万能域名、清空筛选、组合筛选', async ({ contentPage, clearCache, createProject }) => {
    await clearCache();
    await createProject();
    await contentPage.waitForURL(/.*?#?\/workbench/, { timeout: 5000 });
    // 打开Cookie管理页面
  ...
  // 测试用例8: 批量删除Cookie
  test('批量删除Cookie-选中多个删除', async ({ contentPage, clearCache, createProject }) => {
    await clearCache();
    await createProject();
    await contentPage.waitForURL(/.*?#?\/workbench/, { timeout: 5000 });
    // 打开Cookie管理页面
  ...
  test('新增Cookie表单验证-名称为空时不允许保存', async ({ contentPage, clearCache, createProject }) => {
    await clearCache();
    await createProject();
    await contentPage.waitForURL(/.*?#?\/workbench/, { timeout: 5000 });
    // 打开Cookie管理页面
  ...
  test('新增Cookie表单验证-值为空时不允许保存', async ({ contentPage, clearCache, createProject }) => {
    await clearCache();
    await createProject();
    await contentPage.waitForURL(/.*?#?\/workbench/, { timeout: 5000 });
    // 打开Cookie管理页面
  ...
  test('新增Cookie-填写所有属性验证列表完整展示', async ({ contentPage, clearCache, createProject }) => {
    await clearCache();
    await createProject();
    await contentPage.waitForURL(/.*?#?\/workbench/, { timeout: 5000 });
    // 打开Cookie管理页面
  ...
  test('新增Cookie-仅填写必填项验证默认值展示', async ({ contentPage, clearCache, createProject }) => {
    await clearCache();
    await createProject();
    await contentPage.waitForURL(/.*?#?\/workbench/, { timeout: 5000 });
    // 打开Cookie管理页面
