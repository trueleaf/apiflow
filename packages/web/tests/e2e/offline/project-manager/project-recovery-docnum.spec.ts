...
test.describe('ProjectRecoveryDocNum', () => {
  test('恢复项目后接口数量正确展示，folder节点不计入接口总数', async ({ topBarPage, contentPage, clearCache, createProject }) => {
    await clearCache()
    ...
    await addFolderDialog.locator('[data-testid="add-folder-name-input"] input').fill(`Folder-${Date.now()}`)
    ...
    await addApiBtn.click()
    await expect(addApiDialog).toBeVisible({ timeout: 5000 })
    await addApiDialog.locator('input').first().fill(`WebSocket-${Date.now()}`)
    await addApiDialog.locator('.el-radio').filter({ hasText: /^WebSocket$/ }).click()
    await addApiDialog.locator('.el-dialog__footer .el-button--primary').click()
    await expect(addApiDialog).toBeHidden({ timeout: 5000 })
    await addApiBtn.click()
    await expect(addApiDialog).toBeVisible({ timeout: 5000 })
    await addApiDialog.locator('input').first().fill(`HTTPMock-${Date.now()}`)
    await addApiDialog.locator('.el-radio').filter({ hasText: 'HTTP Mock' }).click()
    await addApiDialog.locator('.el-dialog__footer .el-button--primary').click()
    await expect(addApiDialog).toBeHidden({ timeout: 5000 })
    await addApiBtn.click()
    await expect(addApiDialog).toBeVisible({ timeout: 5000 })
    await addApiDialog.locator('input').first().fill(`WebSocketMock-${Date.now()}`)
