...
test.describe('RequestUrlUndo', () => {
  ...
  test('输入字符串后点击撤销按钮逐步撤销', async ({ contentPage, clearCache, createProject }) => {
    ...
    await contentPage.waitForTimeout(200);
    // 验证当前url值为ab
    await expect(urlInput).toHaveValue('ab', { timeout: 5000 });
    // 点击撤销按钮
    const undoBtn = contentPage.locator('.undo-btn, [data-testid="undo-btn"]').first();
    await undoBtn.click();
    await contentPage.waitForTimeout(200);
    // 验证url值为a
    await expect(urlInput).toHaveValue('a', { timeout: 5000 });
    // 再次点击撤销按钮
    await undoBtn.click();
    await contentPage.waitForTimeout(200);
    // 验证url值为空
  ...
  test('输入中文字符串后撤销', async ({ contentPage, clearCache, createProject }) => {
    ...
    await contentPage.waitForTimeout(200);
    // 验证当前url值为你好
    await expect(urlInput).toHaveValue('你好', { timeout: 5000 });
    // 按ctrl+z快捷键撤销
    await contentPage.keyboard.press('Control+z');
    await contentPage.waitForTimeout(200);
    // 验证url值为空
  ...
  test('输入字符后粘贴内容再撤销', async ({ contentPage, clearCache, createProject }) => {
    ...
    await contentPage.waitForTimeout(200);
    // 验证当前url值
    await expect(urlInput).toHaveValue('atest.demo.com', { timeout: 5000 });
    // 按ctrl+z快捷键撤销
    await contentPage.keyboard.press('Control+z');
    await contentPage.waitForTimeout(200);
    // 验证url值恢复为a
