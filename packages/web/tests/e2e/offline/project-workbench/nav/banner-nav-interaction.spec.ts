...
test.describe('BannerNavInteraction', () => {
  ...
  test('单击左侧非folder类型节点新增非固定tab页签且字体斜体', async ({ contentPage, clearCache, createProject, createNode }) => {
    await clearCache();
    await createProject();
  ...
  test('双击左侧非folder类型节点新增固定tab页签且字体正常', async ({ contentPage, clearCache, createProject, createNode }) => {
    await clearCache();
    await createProject();
    await contentPage.waitForURL(/.*?#?\/workbench/, { timeout: 5000 });
    // 新增HTTP节点
  ...
  test('单击左侧folder类型节点不会新增tab页签', async ({ contentPage, clearCache, createProject, createNode }) => {
    await clearCache();
    await createProject();
    await contentPage.waitForURL(/.*?#?\/workbench/, { timeout: 5000 });
    // 新增文件夹节点
  ...
  test('单击非folder节点B覆盖非固定tab节点A', async ({ contentPage, clearCache, createProject, createNode }) => {
    await clearCache();
    await createProject();
    await contentPage.waitForURL(/.*?#?\/workbench/, { timeout: 5000 });
    // 新增两个HTTP节点
  ...
  test('双击非folder节点覆盖非固定页签并设为固定', async ({ contentPage, clearCache, createProject, createNode }) => {
    await clearCache();
    await createProject();
    await contentPage.waitForURL(/.*?#?\/workbench/, { timeout: 5000 });
    // 新增两个HTTP节点
  ...
  test('banner新增节点会在右侧导航栏新增固定tab页签', async ({ contentPage, clearCache, createProject, createNode }) => {
    await clearCache();
    await createProject();
    await contentPage.waitForURL(/.*?#?\/workbench/, { timeout: 5000 });
    // 新增HTTP节点
  ...
  test('删除banner节点右侧导航栏删除对应tab页签', async ({ contentPage, clearCache, createProject, createNode }) => {
    await clearCache();
    await createProject();
    await contentPage.waitForURL(/.*?#?\/workbench/, { timeout: 5000 });
    // 新增HTTP节点
    ...
    // 验证tab不存在
    await expect(navTab).not.toBeVisible({ timeout: 3000 });
  });
  // 测试用例8: 重命名banner节点，右侧导航栏对应tab页签名称更新
  test('重命名banner节点右侧导航栏对应tab页签名称更新', async ({ contentPage, clearCache, createProject, createNode }) => {
    await clearCache();
    await createProject();
    await contentPage.waitForURL(/.*?#?\/workbench/, { timeout: 5000 });
    // 新增HTTP节点
  ...
  test('点击tab页签左侧banner节点高亮', async ({ contentPage, clearCache, createProject, createNode }) => {
    await clearCache();
    await createProject();
    await contentPage.waitForURL(/.*?#?\/workbench/, { timeout: 5000 });
    // 新增两个HTTP节点
  ...
  // 测试用例10: 单击非固定页签变为固定
  test('双击非固定页签变为固定', async ({ contentPage, clearCache, createProject, createNode }) => {
    await clearCache();
    await createProject();
    await contentPage.waitForURL(/.*?#?\/workbench/, { timeout: 5000 });
    // 新增HTTP节点
