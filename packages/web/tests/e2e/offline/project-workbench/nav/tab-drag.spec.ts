...
test.describe('TabDrag', () => {
  // 测试用例1: 拖拽tab页签改变顺序
  test('拖拽tab页签改变顺序', async ({ contentPage, clearCache, createProject, createNode }) => {
    await clearCache();
    await createProject();
  ...
  test('拖拽tab页签后激活状态保持', async ({ contentPage, clearCache, createProject, createNode }) => {
    await clearCache();
    await createProject();
    await contentPage.waitForURL(/.*?#?\/workbench/, { timeout: 5000 });
    // 新增两个HTTP节点
  ...
  // 测试用例3: 拖拽后页签数量不变
  test('拖拽后页签数量不变', async ({ contentPage, clearCache, createProject, createNode }) => {
    await clearCache();
    await createProject();
    await contentPage.waitForURL(/.*?#?\/workbench/, { timeout: 5000 });
    // 新增三个HTTP节点
    for (let i = 1; i <= 3; i++) {
      await createNode(contentPage, { nodeType: 'http', name: `数量测试${i}` });
    }
    // 获取初始tab数量
  ...
  // 测试用例4: 拖拽单个tab时无变化
  test('只有单个tab时拖拽无变化', async ({ contentPage, clearCache, createProject, createNode }) => {
    await clearCache();
    await createProject();
    await contentPage.waitForURL(/.*?#?\/workbench/, { timeout: 5000 });
    // 新增一个HTTP节点
  ...
  // 测试用例5: 拖拽后固定状态保持
  test('拖拽后固定状态保持', async ({ contentPage, clearCache, createProject, createNode }) => {
    await clearCache();
    await createProject();
    await contentPage.waitForURL(/.*?#?\/workbench/, { timeout: 5000 });
    // 新增两个HTTP节点（新增时为固定状态）
