...
test.describe('DeleteNode', () => {
  test.describe('删除httpNode节点', () => {
    ...
    test('按住ctrl鼠标左键批量选择httpNode节点,鼠标右键批量删除', async ({ contentPage, clearCache, createProject }) => {
      ...
      for (let i = 1; i <= 3; i++) {
        ...
        const httpNode = contentPage.locator('.el-tree-node__content').filter({ hasText: `批量删除HTTP节点${i}` });
  ...
  test.describe('删除websocketNode节点', () => {
    ...
    test('按住ctrl鼠标左键批量选择websocketNode节点,鼠标右键批量删除', async ({ contentPage, clearCache, createProject, createNode }) => {
      await clearCache();
      await createProject();
      await contentPage.waitForURL(/.*?#?\/workbench/, { timeout: 5000 });
      // 使用createNode fixture创建3个WebSocket节点
      for (let i = 1; i <= 3; i++) {
        await createNode(contentPage, { nodeType: 'websocket', name: `批量删除WS节点${i}` });
      }
  ...
  test.describe('删除websocketMockNode节点', () => {
    ...
    test('按住ctrl鼠标左键批量选择websocketMockNode节点,鼠标右键批量删除', async ({ contentPage, clearCache, createProject }) => {
      ...
      for (let i = 1; i <= 3; i++) {
        ...
        await confirmBtn.click();
        await expect(addFileDialog).toBeHidden({ timeout: 10000 });
        const wsMockNode = contentPage.locator('.el-tree-node__content').filter({ hasText: `批量删除WsMock${i}` });
