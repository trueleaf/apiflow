...
test.describe('RenameNode', () => {
  test.describe('重命名httpNode节点', () => {
    ...
    test('修改名称末尾后点击输入框前部保留修改内容', async ({ contentPage, clearCache, createProject }) => {
      await clearCache();
      await createProject();
      await contentPage.waitForURL(/.*?#?\/workbench/, { timeout: 5000 });
      await contentPage.waitForTimeout(500);
      ...
      await expect(addFileDialog).toBeVisible({ timeout: 5000 });
      const nameInput = addFileDialog.locator('input').first();
      await nameInput.fill('待修改HTTP节点');
      const confirmBtn = addFileDialog.locator('.el-button--primary').last();
      await confirmBtn.click();
      await contentPage.waitForTimeout(500);
      const httpNode = contentPage.locator('.el-tree-node__content').filter({ hasText: '待修改HTTP节点' });
      await expect(httpNode).toBeVisible({ timeout: 5000 });
      await httpNode.click({ button: 'right' });
      await contentPage.waitForTimeout(300);
      const renameItem = contentPage.locator('.s-contextmenu .s-contextmenu-item', { hasText: /重命名/ });
      ...
      await renameInput.pressSequentially('追加内容');
      await expect(renameInput).toHaveValue('待修改HTTP节点追加内容');
      await renameInput.click({ position: { x: 5, y: 5 } });
      await expect(renameInput).toHaveValue('待修改HTTP节点追加内容');
    });
