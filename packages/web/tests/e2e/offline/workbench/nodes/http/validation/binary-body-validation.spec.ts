...
test.describe('BinaryBodyValidation', () => {
  ...
  test('切换Body类型为Binary后显示Binary选项', async ({ contentPage, clearCache, createProject, createNode }) => {
    await clearCache();
    await createProject();
  ...
  test('Binary变量模式输入框可见', async ({ contentPage, clearCache, createProject, createNode }) => {
    await clearCache();
    await createProject();
    await contentPage.waitForURL(/.*?#?\/workbench/, { timeout: 10000 });
  ...
  test('Binary变量模式可以输入变量', async ({ contentPage, clearCache, createProject, createNode }) => {
    await clearCache();
    await createProject();
    await contentPage.waitForURL(/.*?#?\/workbench/, { timeout: 10000 });
  ...
  test('Binary文件模式选择文件按钮可见', async ({ contentPage, clearCache, createProject, createNode }) => {
    await clearCache();
    await createProject();
    await contentPage.waitForURL(/.*?#?\/workbench/, { timeout: 10000 });
  ...
  test('Binary模式切换在变量和文件之间', async ({ contentPage, clearCache, createProject, createNode }) => {
    await clearCache();
    await createProject();
    await contentPage.waitForURL(/.*?#?\/workbench/, { timeout: 10000 });
  ...
  test('POST方法下可以选择Binary类型', async ({ contentPage, clearCache, createProject, createNode }) => {
    await clearCache();
    await createProject();
    await contentPage.waitForURL(/.*?#?\/workbench/, { timeout: 10000 });
  ...
  test('PUT方法下可以选择Binary类型', async ({ contentPage, clearCache, createProject, createNode }) => {
    await clearCache();
    await createProject();
    await contentPage.waitForURL(/.*?#?\/workbench/, { timeout: 10000 });
  ...
  test('PATCH方法下可以选择Binary类型', async ({ contentPage, clearCache, createProject, createNode }) => {
    await clearCache();
    await createProject();
    await contentPage.waitForURL(/.*?#?\/workbench/, { timeout: 10000 });
  ...
  // Binary 变量值在标签切换后应保持
  test('Binary变量模式输入值在切换tab后保持', async ({ contentPage, clearCache, createProject, createNode }) => {
    await clearCache();
    await createProject();
    await contentPage.waitForURL(/.*?#?\/workbench/, { timeout: 10000 });
  ...
  test('Binary模式选择在切换tab后保持', async ({ contentPage, clearCache, createProject, createNode }) => {
    await clearCache();
    await createProject();
    await contentPage.waitForURL(/.*?#?\/workbench/, { timeout: 10000 });
  ...
  test('从Binary切换到JSON类型后再切回Binary', async ({ contentPage, clearCache, createProject, createNode }) => {
    await clearCache();
    await createProject();
    await contentPage.waitForURL(/.*?#?\/workbench/, { timeout: 10000 });
  ...
  test('Binary变量模式输入框有正确的placeholder提示', async ({ contentPage, clearCache, createProject, createNode }) => {
    await clearCache();
    await createProject();
    await contentPage.waitForURL(/.*?#?\/workbench/, { timeout: 10000 });
  ...
  // Binary 变量模式请求发送后应返回可见响应
  test('调用echo接口验证Binary变量模式请求是否正常发送', async ({ contentPage, clearCache, createProject, createNode }) => {
    await clearCache();
    await createProject();
    await contentPage.waitForURL(/.*?#?\/workbench/, { timeout: 10000 });
    await createNode(contentPage, { nodeType: 'http', name: 'Binary发送测试' });
    // 配置 echo 地址和 POST 请求
    ...
    await contentPage.locator('.body-params .el-radio', { hasText: /binary/i }).first().click();
    await contentPage.locator('.binary-wrap .el-radio', { hasText: /变量模式/ }).click();
    await contentPage.locator('.var-mode .el-input input').fill('test binary content');
    // 发送请求并校验响应区域状态
