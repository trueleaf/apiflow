...
test.describe('Logo', () => {
  test('点击logo跳转主页面', async ({ topBarPage, contentPage, clearCache, loginAccount, jumpToSettings }) => {
    await clearCache();
    await loginAccount();
  ...
  test('设置页面更改应用图标后logo立马被更新, 刷新页面保持更新后的图标', async ({ topBarPage, contentPage, clearCache, loginAccount, reload, jumpToSettings }) => {
    await clearCache();
    await loginAccount();
    // 获取初始Logo的src
    const logo = topBarPage.locator('.logo-img');
  ...
  test('点击重置后，图标恢复为默认', async ({ topBarPage, contentPage, clearCache, loginAccount, jumpToSettings }) => {
    await clearCache();
    await loginAccount();
    // 先上传自定义图标
    const logo = topBarPage.locator('.logo-img');
