...
test.describe('OnlineWebSocketDeleteNode', () => {
  test('创建 WebSocket 节点后删除节点', async ({ topBarPage, contentPage, clearCache }) => {
    ...
    const projectName = `在线WS删除项目-${Date.now()}`;
    ...
    await expect(confirmDialog.locator('.el-message-box__message')).toContainText('待删除Websocket节点');
    await confirmDialog.locator('.el-button--primary').click();

    await contentPage.waitForTimeout(500);
    await expect(contentPage.locator('.el-tree-node__content').filter({ hasText: '待删除Websocket节点' })).toBeHidden({ timeout: 10000 });
  });
});
