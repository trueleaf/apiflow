...
test.describe('OnlineFormdataBodyValidation', () => {
  test('调用echo接口验证常规formdata是否正常返回,content-type是否设置正确', async ({ topBarPage, contentPage, clearCache }) => {
    ...
    const projectName = `在线FormData-${Date.now()}`;
    ...
    await addFileDialog.locator('input').first().fill('FormData测试接口');
  ...
  test('调用echo接口验证使用变量的formdata是否正常返回,content-type是否设置正确', async ({ topBarPage, contentPage, clearCache }) => {
    const serverUrl = process.env.TEST_SERVER_URL;
    const loginName = process.env.TEST_LOGIN_NAME;
    const password = process.env.TEST_LOGIN_PASSWORD;
    const captcha = process.env.TEST_LOGIN_CAPTCHA;
  ...
  test('调用echo接口验证使用mock的formdata是否正常返回,content-type是否设置正确', async ({ topBarPage, contentPage, clearCache }) => {
    const serverUrl = process.env.TEST_SERVER_URL;
    const loginName = process.env.TEST_LOGIN_NAME;
    const password = process.env.TEST_LOGIN_PASSWORD;
    const captcha = process.env.TEST_LOGIN_CAPTCHA;
