...
test.describe('AfLocalStorageApi', () => {
  ...
  test('使用af.localStorage.set(key, value)存储持久数据', async ({ contentPage }) => {
    // 新增HTTP节点
  ...
  test('使用af.localStorage.get(key)获取持久数据', async ({ contentPage }) => {
    // 预先设置localStorage数据
  ...
  test('使用af.localStorage.remove(key)删除持久数据', async ({ contentPage }) => {
    // 预先设置localStorage数据
    await contentPage.evaluate(() => {
  ...
  test('使用af.localStorage.clear()清空所有持久数据', async ({ contentPage }) => {
  ...
  // 测试用例5: 获取不存在的键返回null
  test('获取不存在的键返回null', async ({ contentPage }) => {
    // 新增HTTP节点
    const addFileBtn = contentPage.locator('[data-testid="banner-add-http-btn"]');
    await addFileBtn.click();
    const addFileDialog = contentPage.locator('[data-testid="add-file-dialog"]');
