...
test.describe('PreScriptEditorFeatures', () => {
  ...
  test('前置脚本编辑器支持JavaScript语法高亮', async ({ contentPage, clearCache, createProject, createNode, loginAccount }) => {
    await clearCache();
  ...
  // 输入af.后出现代码补全提示
  test('输入af后出现代码补全提示', async ({ contentPage, clearCache, createProject, createNode, loginAccount }) => {
    await clearCache();
  ...
  test('鼠标悬停在af对象属性上显示API说明提示', async ({ contentPage, clearCache, createProject, createNode, loginAccount }) => {
    await clearCache();
