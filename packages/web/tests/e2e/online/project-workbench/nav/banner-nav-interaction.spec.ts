...
test.describe('OnlineBannerNavInteraction', () => {
  test('单击非 folder 节点新增非固定 tab', async ({ topBarPage, contentPage, clearCache }) => {
    ...
    const projectName = `在线单击节点-${Date.now()}`;
    ...
    await addFileDialog.locator('input').first().fill('单击节点接口');
    ...
    const bannerNode = contentPage.locator('.el-tree-node__content').filter({ hasText: '单击节点接口' }).first();
  ...
  test('双击非 folder 节点新增固定 tab', async ({ topBarPage, contentPage, clearCache }) => {
    const serverUrl = process.env.TEST_SERVER_URL;
    const loginName = process.env.TEST_LOGIN_NAME;
    const password = process.env.TEST_LOGIN_PASSWORD;
    const captcha = process.env.TEST_LOGIN_CAPTCHA;
    ...
    await contentPage.waitForURL(/.*#\/home.*/, { timeout: 10000 });
    const projectName = `在线双击节点-${Date.now()}`;
    await contentPage.locator('[data-testid="home-add-project-btn"]').click();
    const projectDialog = contentPage.locator('.el-dialog').filter({ hasText: /新建项目|新增项目|Create Project/ });
    await expect(projectDialog).toBeVisible({ timeout: 5000 });
    await projectDialog.locator('input').first().fill(projectName);
    ...
    const addFileDialog = contentPage.getByTestId('add-file-dialog');
    await expect(addFileDialog).toBeVisible({ timeout: 5000 });
    await addFileDialog.locator('input').first().fill('双击节点接口');
    await addFileDialog.locator('.el-button--primary').last().click();
    await contentPage.waitForTimeout(500);
    const closeBtn = contentPage.locator('[data-testid="project-nav-tab-close-btn"]').first();
    await closeBtn.click();
    await contentPage.waitForTimeout(300);
    const bannerNode = contentPage.locator('.el-tree-node__content').filter({ hasText: '双击节点接口' }).first();
  ...
  test('单击 folder 节点不会新增 tab', async ({ topBarPage, contentPage, clearCache }) => {
    const serverUrl = process.env.TEST_SERVER_URL;
    const loginName = process.env.TEST_LOGIN_NAME;
    const password = process.env.TEST_LOGIN_PASSWORD;
    const captcha = process.env.TEST_LOGIN_CAPTCHA;
    ...
    await contentPage.waitForURL(/.*#\/home.*/, { timeout: 10000 });
    const projectName = `在线folder节点-${Date.now()}`;
    await contentPage.locator('[data-testid="home-add-project-btn"]').click();
    const projectDialog = contentPage.locator('.el-dialog').filter({ hasText: /新建项目|新增项目|Create Project/ });
    await expect(projectDialog).toBeVisible({ timeout: 5000 });
    await projectDialog.locator('input').first().fill(projectName);
    ...
    await addFolderDialog.locator('[data-testid="add-folder-name-input"] input').fill('不新增tab文件夹');
    ...
    const folderNode = contentPage.locator('.el-tree-node__content').filter({ hasText: '不新增tab文件夹' }).first();
