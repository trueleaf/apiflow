...
test.describe('BannerNavInteraction', () => {
  ...
  test('单击左侧非folder类型节点新增非固定tab页签且字体斜体', async ({ contentPage, clearCache, createProject, loginAccount, createNode }) => {
    await clearCache();
    ...
    // 新增HTTP节点
    await createNode(contentPage, { nodeType: 'http', name: '测试接口1' });
    // 关闭当前tab（双击创建的是固定tab）
  ...
  test('双击左侧非folder类型节点新增固定tab页签且字体正常', async ({ contentPage, clearCache, createProject, loginAccount, createNode }) => {
    await clearCache();
    ...
    await contentPage.waitForURL(/.*?#?\/workbench/, { timeout: 5000 });
    // 新增HTTP节点
    await createNode(contentPage, { nodeType: 'http', name: '测试接口2' });
    // 关闭当前tab
    const closeBtn = contentPage.locator('[data-testid="project-nav-tab-close-btn"]').first();
    await closeBtn.click();
    await contentPage.waitForTimeout(300);
  ...
  test('单击左侧folder类型节点不会新增tab页签', async ({ contentPage, clearCache, createProject, loginAccount, createNode }) => {
    await clearCache();
    ...
    await contentPage.waitForURL(/.*?#?\/workbench/, { timeout: 5000 });
    // 新增文件夹节点
    await createNode(contentPage, { nodeType: 'folder', name: '测试文件夹' });
    // 获取当前tab数量
  ...
  test('单击非folder节点B覆盖非固定tab节点A', async ({ contentPage, clearCache, createProject, loginAccount, createNode }) => {
    await clearCache();
    ...
    await contentPage.waitForURL(/.*?#?\/workbench/, { timeout: 5000 });
    // 新增两个HTTP节点
    await createNode(contentPage, { nodeType: 'http', name: '接口A' });
    // 关闭当前tab
    const closeBtn = contentPage.locator('[data-testid="project-nav-tab-close-btn"]').first();
    await closeBtn.click();
    await contentPage.waitForTimeout(300);
    // 新增第二个节点
    await createNode(contentPage, { nodeType: 'http', name: '接口B' });
    // 关闭当前tab
  ...
  test('双击非folder节点覆盖非固定页签并设为固定', async ({ contentPage, clearCache, createProject, loginAccount, createNode }) => {
    await clearCache();
    ...
    await contentPage.waitForURL(/.*?#?\/workbench/, { timeout: 5000 });
    // 新增两个HTTP节点
    await createNode(contentPage, { nodeType: 'http', name: '接口C' });
    // 关闭当前固定tab
    await contentPage.locator('[data-testid="project-nav-tab-close-btn"]').first().click();
    await contentPage.waitForTimeout(300);
    // 新增第二个节点
    await createNode(contentPage, { nodeType: 'http', name: '接口D' });
    // 关闭当前tab
    await contentPage.locator('[data-testid="project-nav-tab-close-btn"]').first().click();
    await contentPage.waitForTimeout(300);
    // 单击节点C产生非固定页签
  ...
  test('banner新增节点会在右侧导航栏新增固定tab页签', async ({ contentPage, clearCache, createProject, loginAccount, createNode }) => {
    await clearCache();
    ...
    await contentPage.waitForURL(/.*?#?\/workbench/, { timeout: 5000 });
    // 新增HTTP节点
    await createNode(contentPage, { nodeType: 'http', name: '新增测试接口' });
    // 验证tab页签存在且为固定状态
    const navTab = contentPage.locator('.nav .item.active');
    await expect(navTab).toBeVisible({ timeout: 3000 });
  ...
  test('删除banner节点右侧导航栏删除对应tab页签', async ({ contentPage, clearCache, createProject, loginAccount, createNode }) => {
    await clearCache();
  ...
  test('重命名banner节点右侧导航栏对应tab页签名称更新', async ({ contentPage, clearCache, createProject, loginAccount, createNode }) => {
    await clearCache();
    ...
    await contentPage.waitForURL(/.*?#?\/workbench/, { timeout: 5000 });
    // 新增HTTP节点
    await createNode(contentPage, { nodeType: 'http', name: '原接口名称' });
    // 验证tab存在
    const navTab = contentPage.locator('.nav .item.active');
  ...
  test('点击tab页签左侧banner节点高亮', async ({ contentPage, clearCache, createProject, loginAccount, createNode }) => {
    await clearCache();
    ...
    await contentPage.waitForURL(/.*?#?\/workbench/, { timeout: 5000 });
    // 新增两个HTTP节点
    await createNode(contentPage, { nodeType: 'http', name: '接口E' });
    // 新增第二个节点
    await createNode(contentPage, { nodeType: 'http', name: '接口F' });
    // 点击第一个tab
  ...
  // 测试用例10: 单击非固定页签变为固定
  test('双击非固定页签变为固定', async ({ contentPage, clearCache, createProject, loginAccount, createNode }) => {
    await clearCache();
    ...
    await contentPage.waitForURL(/.*?#?\/workbench/, { timeout: 5000 });
    // 新增HTTP节点
    await createNode(contentPage, { nodeType: 'http', name: '测试接口G' });
    // 关闭固定tab
    await contentPage.locator('[data-testid="project-nav-tab-close-btn"]').first().click();
    await contentPage.waitForTimeout(300);
    // 单击产生非固定tab
