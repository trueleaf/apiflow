...
test.describe('NavBasicStyle', () => {
  ...
  // 测试用例2: tab页签激活状态样式正确
  test('tab页签激活状态样式正确', async ({ contentPage, clearCache, createProject, loginAccount, createNode }) => {
    await clearCache();
  ...
  // 测试用例3: 非固定页签显示斜体样式
  test('非固定页签显示斜体样式', async ({ contentPage, clearCache, createProject, loginAccount, createNode }) => {
    await clearCache();
  ...
  // 测试用例4: 固定页签显示正常样式
  test('固定页签显示正常样式', async ({ contentPage, clearCache, createProject, loginAccount, createNode }) => {
    await clearCache();
  ...
  // 测试用例5: 未保存状态显示小圆点
  test('未保存状态显示小圆点', async ({ contentPage, clearCache, createProject, loginAccount, createNode }) => {
    await clearCache();
  ...
  test('HTTP节点tab显示请求方法标识', async ({ contentPage, clearCache, createProject, loginAccount, createNode }) => {
    await clearCache();
  ...
  // 测试用例7: 页签文本过长显示省略号
  test('页签文本过长显示省略号', async ({ contentPage, clearCache, createProject, loginAccount, createNode }) => {
    await clearCache();
