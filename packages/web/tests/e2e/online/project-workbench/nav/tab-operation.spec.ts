...
test.describe('OnlineTabOperation', () => {
  test('点击关闭按钮关闭当前 tab', async ({ topBarPage, contentPage, clearCache }) => {
    ...
    const projectName = `在线关闭tab-${Date.now()}`;
    ...
    await addFileDialog.locator('input').first().fill('关闭tab接口');
    await addFileDialog.locator('.el-button--primary').last().click();
    await contentPage.waitForTimeout(500);
    const tab = contentPage.locator('.nav .item').filter({ hasText: '关闭tab接口' });
  ...
  test('右键菜单关闭当前 tab', async ({ topBarPage, contentPage, clearCache }) => {
    const serverUrl = process.env.TEST_SERVER_URL;
    const loginName = process.env.TEST_LOGIN_NAME;
    const password = process.env.TEST_LOGIN_PASSWORD;
    const captcha = process.env.TEST_LOGIN_CAPTCHA;
    ...
    await contentPage.waitForURL(/.*#\/home.*/, { timeout: 10000 });
    const projectName = `在线右键关闭-${Date.now()}`;
    await contentPage.locator('[data-testid="home-add-project-btn"]').click();
    const projectDialog = contentPage.locator('.el-dialog').filter({ hasText: /新建项目|新增项目|Create Project/ });
    await expect(projectDialog).toBeVisible({ timeout: 5000 });
    await projectDialog.locator('input').first().fill(projectName);
    ...
    const addFileDialog = contentPage.getByTestId('add-file-dialog');
    await expect(addFileDialog).toBeVisible({ timeout: 5000 });
    await addFileDialog.locator('input').first().fill('右键关闭接口');
    await addFileDialog.locator('.el-button--primary').last().click();
    await contentPage.waitForTimeout(500);
    const tab = contentPage.locator('.nav .item').filter({ hasText: '右键关闭接口' });
    await expect(tab).toBeVisible({ timeout: 3000 });
    await tab.click({ button: 'right' });
    await contentPage.waitForTimeout(300);
    const closeMenuItem = contentPage.locator('[data-testid="contextmenu-item-关闭"], [data-testid="contextmenu-item-Close"]');
    await expect(closeMenuItem).toBeVisible({ timeout: 3000 });
    await closeMenuItem.click();
    await contentPage.waitForTimeout(300);
    await expect(tab).not.toBeVisible({ timeout: 3000 });
  });

  test('新增空白接口按钮新增 tab', async ({ topBarPage, contentPage, clearCache }) => {
    const serverUrl = process.env.TEST_SERVER_URL;
    const loginName = process.env.TEST_LOGIN_NAME;
    const password = process.env.TEST_LOGIN_PASSWORD;
    const captcha = process.env.TEST_LOGIN_CAPTCHA;
    ...
    await contentPage.waitForURL(/.*#\/home.*/, { timeout: 10000 });
    const projectName = `在线新增tab-${Date.now()}`;
    await contentPage.locator('[data-testid="home-add-project-btn"]').click();
    const projectDialog = contentPage.locator('.el-dialog').filter({ hasText: /新建项目|新增项目|Create Project/ });
    await expect(projectDialog).toBeVisible({ timeout: 5000 });
    await projectDialog.locator('input').first().fill(projectName);
  ...
  test('右键菜单关闭左侧 tab', async ({ topBarPage, contentPage, clearCache }) => {
    const serverUrl = process.env.TEST_SERVER_URL;
    const loginName = process.env.TEST_LOGIN_NAME;
    const password = process.env.TEST_LOGIN_PASSWORD;
    const captcha = process.env.TEST_LOGIN_CAPTCHA;
    ...
    await contentPage.waitForURL(/.*#\/home.*/, { timeout: 10000 });
    const projectName = `在线关闭左侧-${Date.now()}`;
    await contentPage.locator('[data-testid="home-add-project-btn"]').click();
    const projectDialog = contentPage.locator('.el-dialog').filter({ hasText: /新建项目|新增项目|Create Project/ });
    await expect(projectDialog).toBeVisible({ timeout: 5000 });
    await projectDialog.locator('input').first().fill(projectName);
    ...
    for (let i = 1; i <= 3; i++) {
      ...
      const addFileDialog = contentPage.getByTestId('add-file-dialog');
      await expect(addFileDialog).toBeVisible({ timeout: 5000 });
      await addFileDialog.locator('input').first().fill(`左侧接口${i}`);
      await addFileDialog.locator('.el-button--primary').last().click();
      await contentPage.waitForTimeout(500);
    }
    const tab3 = contentPage.locator('.nav .item').filter({ hasText: '左侧接口3' });
    ...
    await expect(closeLeftMenuItem).toBeVisible({ timeout: 3000 });
    await closeLeftMenuItem.click();
    await contentPage.waitForTimeout(300);
    await expect(contentPage.locator('.nav .item').filter({ hasText: '左侧接口1' })).not.toBeVisible();
    await expect(contentPage.locator('.nav .item').filter({ hasText: '左侧接口2' })).not.toBeVisible();
    await expect(tab3).toBeVisible();
  });

  test('右键菜单关闭右侧 tab', async ({ topBarPage, contentPage, clearCache }) => {
    const serverUrl = process.env.TEST_SERVER_URL;
    const loginName = process.env.TEST_LOGIN_NAME;
    const password = process.env.TEST_LOGIN_PASSWORD;
    const captcha = process.env.TEST_LOGIN_CAPTCHA;
    ...
    await contentPage.waitForURL(/.*#\/home.*/, { timeout: 10000 });
    const projectName = `在线关闭右侧-${Date.now()}`;
    await contentPage.locator('[data-testid="home-add-project-btn"]').click();
    const projectDialog = contentPage.locator('.el-dialog').filter({ hasText: /新建项目|新增项目|Create Project/ });
    await expect(projectDialog).toBeVisible({ timeout: 5000 });
    await projectDialog.locator('input').first().fill(projectName);
    ...
    for (let i = 1; i <= 3; i++) {
      ...
      const addFileDialog = contentPage.getByTestId('add-file-dialog');
      await expect(addFileDialog).toBeVisible({ timeout: 5000 });
      await addFileDialog.locator('input').first().fill(`右侧接口${i}`);
      await addFileDialog.locator('.el-button--primary').last().click();
      await contentPage.waitForTimeout(500);
    }
    const tab1 = contentPage.locator('.nav .item').filter({ hasText: '右侧接口1' });
    ...
    await expect(closeRightMenuItem).toBeVisible({ timeout: 3000 });
    await closeRightMenuItem.click();
    await contentPage.waitForTimeout(300);
    await expect(contentPage.locator('.nav .item').filter({ hasText: '右侧接口2' })).not.toBeVisible();
    await expect(contentPage.locator('.nav .item').filter({ hasText: '右侧接口3' })).not.toBeVisible();
    await expect(tab1).toBeVisible();
  });

  test('右键菜单关闭其他 tab', async ({ topBarPage, contentPage, clearCache }) => {
    const serverUrl = process.env.TEST_SERVER_URL;
    const loginName = process.env.TEST_LOGIN_NAME;
    const password = process.env.TEST_LOGIN_PASSWORD;
    const captcha = process.env.TEST_LOGIN_CAPTCHA;
    ...
    await contentPage.waitForURL(/.*#\/home.*/, { timeout: 10000 });
    const projectName = `在线关闭其他-${Date.now()}`;
    await contentPage.locator('[data-testid="home-add-project-btn"]').click();
    const projectDialog = contentPage.locator('.el-dialog').filter({ hasText: /新建项目|新增项目|Create Project/ });
    await expect(projectDialog).toBeVisible({ timeout: 5000 });
    await projectDialog.locator('input').first().fill(projectName);
    ...
    for (let i = 1; i <= 3; i++) {
      ...
      const addFileDialog = contentPage.getByTestId('add-file-dialog');
      await expect(addFileDialog).toBeVisible({ timeout: 5000 });
      await addFileDialog.locator('input').first().fill(`其他接口${i}`);
      await addFileDialog.locator('.el-button--primary').last().click();
      await contentPage.waitForTimeout(500);
    }
    const tab2 = contentPage.locator('.nav .item').filter({ hasText: '其他接口2' });
    ...
    await expect(closeOtherMenuItem).toBeVisible({ timeout: 3000 });
    await closeOtherMenuItem.click();
    await contentPage.waitForTimeout(300);
    await expect(contentPage.locator('.nav .item').filter({ hasText: '其他接口1' })).not.toBeVisible();
    await expect(contentPage.locator('.nav .item').filter({ hasText: '其他接口3' })).not.toBeVisible();
    await expect(tab2).toBeVisible();
    const tabCount = await contentPage.locator('.nav .drag-wrap .item').count();
    expect(tabCount).toBe(1);
  });

  test('右键菜单全部关闭', async ({ topBarPage, contentPage, clearCache }) => {
    const serverUrl = process.env.TEST_SERVER_URL;
    const loginName = process.env.TEST_LOGIN_NAME;
    const password = process.env.TEST_LOGIN_PASSWORD;
    const captcha = process.env.TEST_LOGIN_CAPTCHA;
    ...
    await contentPage.waitForURL(/.*#\/home.*/, { timeout: 10000 });
    const projectName = `在线全部关闭-${Date.now()}`;
    await contentPage.locator('[data-testid="home-add-project-btn"]').click();
    const projectDialog = contentPage.locator('.el-dialog').filter({ hasText: /新建项目|新增项目|Create Project/ });
    await expect(projectDialog).toBeVisible({ timeout: 5000 });
    await projectDialog.locator('input').first().fill(projectName);
    ...
    for (let i = 1; i <= 2; i++) {
      ...
      const addFileDialog = contentPage.getByTestId('add-file-dialog');
      await expect(addFileDialog).toBeVisible({ timeout: 5000 });
      await addFileDialog.locator('input').first().fill(`全部关闭接口${i}`);
      await addFileDialog.locator('.el-button--primary').last().click();
      await contentPage.waitForTimeout(500);
    }
    ...
    await expect(closeAllMenuItem).toBeVisible({ timeout: 3000 });
  ...
  test('点击 tab 切换选中状态', async ({ topBarPage, contentPage, clearCache }) => {
    const serverUrl = process.env.TEST_SERVER_URL;
    const loginName = process.env.TEST_LOGIN_NAME;
    const password = process.env.TEST_LOGIN_PASSWORD;
    const captcha = process.env.TEST_LOGIN_CAPTCHA;
    ...
    await contentPage.waitForURL(/.*#\/home.*/, { timeout: 10000 });
    const projectName = `在线切换选中-${Date.now()}`;
    await contentPage.locator('[data-testid="home-add-project-btn"]').click();
    const projectDialog = contentPage.locator('.el-dialog').filter({ hasText: /新建项目|新增项目|Create Project/ });
    await expect(projectDialog).toBeVisible({ timeout: 5000 });
    await projectDialog.locator('input').first().fill(projectName);
  ...
  test('双击 tab 固定非固定页签', async ({ topBarPage, contentPage, clearCache }) => {
    const serverUrl = process.env.TEST_SERVER_URL;
    const loginName = process.env.TEST_LOGIN_NAME;
    const password = process.env.TEST_LOGIN_PASSWORD;
    const captcha = process.env.TEST_LOGIN_CAPTCHA;
    ...
    await contentPage.waitForURL(/.*#\/home.*/, { timeout: 10000 });
    const projectName = `在线双击固定-${Date.now()}`;
    await contentPage.locator('[data-testid="home-add-project-btn"]').click();
    const projectDialog = contentPage.locator('.el-dialog').filter({ hasText: /新建项目|新增项目|Create Project/ });
    await expect(projectDialog).toBeVisible({ timeout: 5000 });
    await projectDialog.locator('input').first().fill(projectName);
