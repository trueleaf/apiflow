...
test.describe('CallHistory', () => {
  ...
  test('切换到调用历史Tab页,验证Tab切换功能', async ({ contentPage, clearCache, createProject, createNode, loginAccount }) => {
    await clearCache();
  ...
  // 测试用例2: 调用历史搜索框功能验证
  test('调用历史搜索框功能验证,包括placeholder和清空按钮', async ({ contentPage, clearCache, createProject, createNode, loginAccount }) => {
    await clearCache();
    ...
    // 验证搜索框placeholder文案（兼容历史文案与新文案）
    const inputElement = searchInput.locator('input');
    await expect(inputElement).toHaveAttribute('placeholder', /过滤历史记录|搜索节点名称或URL/);
    // 验证搜索框左侧显示搜索图标
  ...
  // 测试用例3: 清空所有历史记录功能验证
  test('清空所有历史记录功能验证,包括确认弹窗', async ({ contentPage, clearCache, createProject, createNode, loginAccount }) => {
    await clearCache();
  ...
  // 测试用例4: 历史记录列表展示验证
  test('历史记录列表展示验证,包括空状态', async ({ contentPage, clearCache, createProject, createNode, loginAccount }) => {
    await clearCache();
  ...
  test('点击历史记录项打开对应接口Tab页', async ({ contentPage, clearCache, createProject, createNode, loginAccount }) => {
    await clearCache();
  ...
  test('已删除接口的历史记录标记验证', async ({ contentPage, clearCache, createProject, createNode, loginAccount }) => {
    await clearCache();
  ...
  test('历史记录滚动加载验证', async ({ contentPage, clearCache, createProject, createNode, loginAccount }) => {
    await clearCache();
  ...
  // 测试用例8: 清理已删除接口历史功能验证
  test('清理已删除接口历史功能验证', async ({ contentPage, clearCache, createProject, createNode, loginAccount }) => {
    await clearCache();
