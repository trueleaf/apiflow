...
test.describe('TabOperation', () => {
  ...
  // 测试用例6: 右键菜单全部关闭
  test('右键菜单全部关闭', async ({ contentPage, clearCache, createProject, createNode, loginAccount }) => {
    await clearCache();
  ...
  // 测试用例7: 点击tab切换选中状态
  test('点击tab切换选中状态', async ({ contentPage, clearCache, createProject, createNode, loginAccount }) => {
    await clearCache();
  ...
  // 测试用例8: 新增空白接口按钮功能
  test('新增空白接口按钮功能', async ({ contentPage, clearCache, createProject, createNode, loginAccount }) => {
    await clearCache();
