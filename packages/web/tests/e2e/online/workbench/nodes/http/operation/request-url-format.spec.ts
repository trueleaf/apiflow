...
test.describe('RequestUrlFormat', () => {
  ...
  test('输入URL后blur自动添加http://前缀，撤销不会回到格式化前', async ({ contentPage, clearCache, createProject, createNode, loginAccount }) => {
    ...
    // 新增HTTP节点
    await createNode(contentPage, { nodeType: 'http', name: 'URL格式化测试' });
    // 输入不带协议前缀的URL
  ...
  test('输入带query参数的URL，格式化后query参数被移除且不进入撤销栈', async ({ contentPage, clearCache, createProject, createNode, loginAccount }) => {
    await clearCache();
    await loginAccount();
    await createProject();
    await contentPage.waitForURL(/.*?#?\/workbench/, { timeout: 5000 });
    // 新增HTTP节点
    await createNode(contentPage, { nodeType: 'http', name: 'URL Query格式化测试' });
    // 输入带query参数的URL
    const urlInput = contentPage.locator('[data-testid="url-input"] [contenteditable]');
    await urlInput.click();
  ...
  test('输入已有http://前缀的URL，格式化不会重复添加前缀', async ({ contentPage, clearCache, createProject, createNode, loginAccount }) => {
    await clearCache();
    await loginAccount();
    await createProject();
    await contentPage.waitForURL(/.*?#?\/workbench/, { timeout: 5000 });
    // 新增HTTP节点
    await createNode(contentPage, { nodeType: 'http', name: 'URL重复前缀测试' });
    // 输入已有http://前缀的URL
    const urlInput = contentPage.locator('[data-testid="url-input"] [contenteditable]');
    await urlInput.click();
  ...
  test('输入以/开头的相对路径，格式化不添加协议前缀', async ({ contentPage, clearCache, createProject, createNode, loginAccount }) => {
    await clearCache();
    await loginAccount();
    await createProject();
    await contentPage.waitForURL(/.*?#?\/workbench/, { timeout: 5000 });
    // 新增HTTP节点
    await createNode(contentPage, { nodeType: 'http', name: 'URL相对路径测试' });
    // 输入相对路径
    const urlInput = contentPage.locator('[data-testid="url-input"] [contenteditable]');
    await urlInput.click();
  ...
  test('逐字符输入后格式化，撤销回到逐字符输入前的状态', async ({ contentPage, clearCache, createProject, createNode, loginAccount }) => {
    await clearCache();
    await loginAccount();
    await createProject();
    await contentPage.waitForURL(/.*?#?\/workbench/, { timeout: 5000 });
    // 新增HTTP节点
    await createNode(contentPage, { nodeType: 'http', name: 'URL逐字符格式化测试' });
    // 逐字符输入
    const urlInput = contentPage.locator('[data-testid="url-input"] [contenteditable]');
    await urlInput.click();
