...
test.describe('VariableUsage', () => {
  ...
  test('在url中使用变量语法发送请求时变量被正确替换', async ({ contentPage, clearCache, loginAccount, createProject, createNode }) => {
    await clearCache();
    await loginAccount();
    await createProject();
  ...
  test('在query参数中使用变量发送请求时变量被正确替换', async ({ contentPage, clearCache, loginAccount, createProject, createNode }) => {
    await clearCache();
    await loginAccount();
    await createProject();
    await contentPage.waitForURL(/.*?#?\/workbench/, { timeout: 10000 });
    ...
    await contentPage.keyboard.type(`http://127.0.0.1:${MOCK_SERVER_PORT}/echo?id={{userId}}`);
    await expect(urlInput).toContainText(`http://127.0.0.1:${MOCK_SERVER_PORT}/echo?id={{userId}}`, { timeout: 5000 });
    await contentPage.locator('[data-testid="operation-send-btn"]').click();
    const responseTabs = contentPage.locator('[data-testid="response-tabs"]');
    await expect(responseTabs).toBeVisible({ timeout: 10000 });
  ...
  test('在header参数中使用变量发送请求时变量被正确替换', async ({ contentPage, clearCache, loginAccount, createProject, createNode }) => {
    await clearCache();
    await loginAccount();
    await createProject();
    await contentPage.waitForURL(/.*?#?\/workbench/, { timeout: 10000 });
    ...
    await expect(urlInput).toContainText(`http://127.0.0.1:${MOCK_SERVER_PORT}/echo`, { timeout: 5000 });
  ...
  test('在body json中使用变量发送请求时变量被正确替换', async ({ contentPage, clearCache, loginAccount, createProject, createNode }) => {
    await clearCache();
    await loginAccount();
    await createProject();
    await contentPage.waitForURL(/.*?#?\/workbench/, { timeout: 10000 });
  ...
  test('在URL路径分段中使用变量发送请求时路径参数被正确替换', async ({ contentPage, clearCache, loginAccount, createProject, createNode }) => {
    await clearCache();
    await loginAccount();
    await createProject();
    await contentPage.waitForURL(/.*?#?\/workbench/, { timeout: 10000 });
  ...
  // 变量更新后重发请求应使用新值
  test('修改变量值后重新发送请求使用更新后的值', async ({ contentPage, clearCache, loginAccount, createProject, createNode }) => {
    await clearCache();
    await loginAccount();
    await createProject();
    await contentPage.waitForURL(/.*?#?\/workbench/, { timeout: 10000 });
    ...
    await urlInput.click();
    await contentPage.keyboard.press('ControlOrMeta+a');
    await contentPage.keyboard.type(`http://127.0.0.1:${MOCK_SERVER_PORT}/echo?ver={{apiVer}}`);
    await expect(urlInput).toContainText(`http://127.0.0.1:${MOCK_SERVER_PORT}/echo?ver={{apiVer}}`, { timeout: 5000 });
  ...
  test('同一请求中URL和Header同时使用不同变量均被正确替换', async ({ contentPage, clearCache, loginAccount, createProject, createNode }) => {
    await clearCache();
    await loginAccount();
    await createProject();
    await contentPage.waitForURL(/.*?#?\/workbench/, { timeout: 10000 });
